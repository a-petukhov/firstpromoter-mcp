 ...
 *
 * API response shape: { data: [...promoters], meta: {...} }
 * Each promoter has: name, email, state, profile.website,
 * stats.revenue_amount, stats.sales_count, stats.customers_count,
 * promoter_campaigns[].campaign.name
 */
export function formatPromoters(data: unknown): string {
  ...
  promoters.forEach((p, i) => {
    ...
    // Campaign info (first campaign if multiple)
    if (campaigns && campaigns.length > 0) {
      const camp = campaigns[0].campaign as Record<string, unknown> | undefined;
      lines.push(`   Campaign: ${camp?.name || 'N/A'}`);
    }

    // Performance stats
    if (stats) {
      const revenue = stats.revenue_amount;
      const sales = stats.sales_count ?? 'N/A';
      const customers = stats.customers_count ?? 'N/A';
      ...
      lines.push(`   Revenue: ${revenueStr} | Sales: ${sales} | Customers: ${customers}`);
    }

    lines.push(''); // blank line between promoters
  });

  // Add pagination info if meta is present
  if (raw?.meta) {
